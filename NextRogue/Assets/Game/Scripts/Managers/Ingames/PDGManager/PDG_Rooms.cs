...
public class PDG_Rooms : MonoBehaviour
{
    ...
    public void Initialize() {
        InGameManager.Instance.PDGManager.TilemapVisualizer.SetTVData(_roomProps.Data); 
        CreateRooms();
        SortRooms();
        RoomTypes();
        DecorateRooms();
    ...
    public void CreateRooms() {
        ...
        InGameManager.Instance.PDGManager.TilemapVisualizer.PaintFloorTiles(allTiles);
        _allWalls = WallGenerator.CreateWalls(allTiles, InGameManager.Instance.PDGManager.TilemapVisualizer);
    ...
    public void DecorateEnemyRoom(List<GameObject> props) {
        ...
        for (int i = 0; i < enemyCount; i++) {
            NP_MainController mainController = GameObject.Instantiate(
                ...
                InGameManager.Instance.Enemies
    ...
    public void DecorateExitRoom(List<GameObject> props) { 
        foreach (var item in props) {
            GameObject.Instantiate(
                props[Random.Range(0, props.Count)],
                InGameManager.Instance.Enviroment
        ...
        while (propCount > activeCount) {
            ...
            if (decorate) {
                GameObject.Instantiate(props[Random.Range(0, props.Count)], new Vector3(pos.x, pos.y, 0), Quaternion.identity)
                    .transform.parent = InGameManager.Instance.Enviroment;
                activeCount++;
                _usedPos.Add(pos);
            } 
        }
